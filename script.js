...
const createCard = (book, number) => {
  const cardContainerEl = createEl("div", { className: "card-container" });
  const cardEl = createEl("div", { className: "card" });
  const numEl = createEl("div", { className: "index-num", innerText: number });
  const authorEl = createEl("h3", { className: "author", innerText: book.sub });
  const titleEl = createEl("h2", { className: "title", innerText: book.title });
  const descEl = createEl("p", { className: "desc", innerText: book.text });
  const moreBtnEl = createEl("button", { className: "more-btn" });

  const moreBtnTextEl = createEl("span", {
    className: "more-text",
    innerText: "read more",
  });
  const moreBtnIconEl = createEl("span", {
    className: "material-icons",
    innerText: "arrow_forward",
  });

  cardEl.append(numEl, authorEl, titleEl, descEl);
  moreBtnEl.append(moreBtnTextEl, moreBtnIconEl);
  cardContainerEl.append(cardEl, moreBtnEl);
  return cardContainerEl;
};

const createCardContainer = (cards) => {
  const cardContainerEl = createEl("div", { className: "container" });

  cards.forEach((card, index) =>
    cardContainerEl.append(createCard(card, index + 1))
  );
...
const createHeader = (title) => {
  ...
  const navIconEl = createEl("span", {
    className: "material-icons",
    innerText: "menu",
  });
  headerEl.append(titleEl, navIconEl);
  return headerEl;
};

const main = () => {
  const root = document.querySelector("#root");
  const headerEl = createHeader(books.logo);
  const cardContainerEl = createCardContainer(books.cards);
  root.append(headerEl, cardContainerEl);
};
